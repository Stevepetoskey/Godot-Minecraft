...
var textures = {1:load("res://textures/Blocks/grass_block_side.png"),2:load("res://textures/Blocks/dirt.png"),3:load("res://textures/Blocks/stone.png"),4:load("res://textures/Blocks/oak_log.png"),5:load("res://textures/Blocks/oak_leaves.png"),6:load("res://textures/Blocks/coal_ore.png"),7:load("res://textures/Blocks/iron_ore.png"),8:load("res://textures/Blocks/oak_planks.png"),
9:load("res://textures/items/stick.png"),10:load("res://textures/Blocks/crafting_table_front.png"),11:preload("res://textures/items/wooden_pickaxe.png"),12:preload("res://textures/items/wooden_axe.png"),13:preload("res://textures/Blocks/cobblestone.png"),14:preload("res://textures/items/wooden_shovel.png"),
...
57:preload("res://textures/Blocks/oak_sapling.png"),58:preload("res://textures/Blocks/chest/normal_closed.png"),59:preload("res://textures/items/bucket.png"),60:preload("res://textures/items/water_bucket.png"),61:preload("res://textures/items/arrow.png")}
...
func can_harvest(block):
	var blockStats = get_node("../..").blockData[block]
	var itemStats = [0,"all"]
	if itemData.has(inventory[0][get_node("select").selected]):
		itemStats = itemData[inventory[0][get_node("select").selected]]
	if (blockStats[0] == itemStats[0] and (blockStats[2].has(itemStats[1]) or blockStats[2].has("allType"))) or blockStats[2].has("all"):
		return true
...
func check_time_to_break(block):
	...
	if itemData.has(inventory[0][get_node("select").selected]):
		itemStats = itemData[inventory[0][get_node("select").selected]]
	var multiplier = 1
	if blockStats[0] == itemStats[0]:
		multiplier = toolMultiplier[itemStats[1]]
	var damage = multiplier/ float(blockStats[1])
	if (blockStats[0] == itemStats[0] and (blockStats[2].has(itemStats[1]) or blockStats[2].has("allType"))) or blockStats[2].has("all"):
		damage /= 30
	else:
		damage /= 100
